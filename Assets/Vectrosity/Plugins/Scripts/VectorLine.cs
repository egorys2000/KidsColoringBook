...
public partial class VectorLine 
	...
	public static string Version () {
		...
		public List<Color32> ColorsMemorized = new List<Color32>();
		public List<float> WidthsMemorized = new List<float>();
	...
	//("DrawnLine", new List<Vector2>(), tex, useLineWidth, LineType.Continuous, Joins.Fill);
	...
	protected void SetupLine (string lineName, Texture texture, float width, LineType lineType, Joins joins, bool use2D) {
		...
		m_lineWidths = new float[1];
			m_lineWidths[0] = width * .5f; //here width
		m_lineWidth = width;
		if (!m_is2D) {
	...
	private void Resize () {
		...
		if (m_pointsCount > originalSegmentCount) {
			//SetColor (m_color, originalSegmentCount, GetSegmentNumber()); //m_color here!
	...
	public void SetColors (Color32[] colors, bool memorize = true) 
	{
		if (colors == null) return;

		for (int i = 0; i < colors.Length; i++)
		{ 
			SetColor(colors[i], i, memorize: memorize);
		}
	}
	
	public void SetColor (Color32 color, bool memorize = true) {
		SetColor (color, 0, pointsCount, memorize: memorize);
	}
	
	public void SetColor (Color32 color, int index, bool memorize = true) {
		SetColor (color, index, index, memorize: memorize);
	}
	
	public void SetColor (Color32 color, int startIndex, int endIndex, bool memorize = true) {

			if (memorize) 
			{
				while (ColorsMemorized.Count <= endIndex)
					ColorsMemorized.Add(Color.red);

				//Debug.Log(color);

				for (int i = startIndex; i <= endIndex && i >= 0; i++)
					ColorsMemorized[i] = color;
			} 
	...
	public void SetColors (List<Color32> lineColors) {
		...
		/*if (pointsCount != m_pointsCount) {
				Debug.Log(pointsCount.ToString() + "-_-" + m_pointsCount.ToString());
				Resize();
		}*/ //here
		if (m_lineType != LineType.Points) {
	...
	public void SetWidth (float width, int startIndex, int endIndex) {
			...
			//
			while (WidthsMemorized.Count < endIndex)
				WidthsMemorized.Add(width);
			for (int i = startIndex; i < endIndex; i++)
				WidthsMemorized[i] = width;
			//
